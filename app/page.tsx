...
export default function Page() {
  ...
  const epochModelIds = runtime?.multi_model_ledger?.current_epoch?.model_ids ?? runtime?.multi_model_portfolio?.eligible_profitable_models ?? [];
  const epochModelNames = epochModelIds.map(modelId => modelLabel(modelId));
  const epochModelsLabel = epochModelNames.length > 0 ? epochModelNames.join(" · ") : "Допущенных кандидатов пока нет";
  const leader = leaderboard.find(item => item.model_id === runtime?.factor_model_tournament?.leader_model_id) ?? leaderboard[0];
  ...
  const leaderName = leader?.display_name ?? paper?.display_name ?? "Ведущая модель пока не выбрана";
  const leaderCompleted = leader?.completed_trades ?? governor?.total_completed_trades ?? 0;
  const leaderDrawdown = leader?.portfolio_max_drawdown ?? paper?.portfolio?.max_drawdown;
  ...
  const mainStatus = automation?.requires_attention ? { title: "Atlas ждёт вашего решения", tone: "warning" as Tone } : !sourcesOk ? { title: "Atlas ждёт данные", tone: "neutral" as Tone } : live ? { title: "Atlas торгует", tone: "positive" as Tone } : recoveringLoss ? { title: "Atlas заменяет убыточный портфель", tone: "warning" as Tone } : validatedProfitableCandidates > 0 || ready ? { title: "Есть подтверждённая модель", tone: "positive" as Tone } : leaderProfitable ? { title: "Есть предварительно прибыльная модель", tone: "warning" as Tone } : stopped && activeStrategies === 0 ? { title: "Atlas ищет новую стратегию", tone: "warning" as Tone } : { title: "Atlas ищет прибыльную модель", tone: "warning" as Tone };
  const statusExplanation = live
    ? "Реальная торговля активна. Следите за результатом и риском."
    : validatedProfitableCandidates > 0
      ? "Реальные ордера отключены. Подтверждённый кандидат ожидает следующих проверок безопасности."
      : leaderProfitable
        ? `Реальные ордера отключены. Собрано ${roundTripProgress} полных сделок портфельной проверки.`
        : "Реальные ордера отключены. Доказанной прибыльной модели пока нет.";
  ...
  return <main><div className="appShell">
    ...
    {tab === "home" && <div className="page homePage">
      ...
      <section className={`statusCard ${mainStatus.tone}`}><div><span className="eyebrow">ГЛАВНЫЙ СТАТУС</span><h1>{mainStatus.title}</h1><p>{statusExplanation}</p></div><span className="statusMark" aria-hidden="true">{live || validatedProfitableCandidates > 0 || ready ? "✓" : "●"}</span></section>

      <section className="section resultCard"><div className="sectionHead"><div><span className="eyebrow">ГЛАВНЫЙ ОРИЕНТИР</span><h2>Портфель кандидатов к торговле</h2></div><Badge status={validatedProfitableCandidates > 0 ? "PASS" : epochModelIds.length > 0 ? "PROMISING" : "COLLECTING_DATA"} label={validatedProfitableCandidates > 0 ? "Есть подтверждение" : epochModelIds.length > 0 ? "Предварительно, не подтверждён" : "Идёт отбор"} /></div><div className="primaryOutcome"><div className="leaderIdentity"><b>{epochModelsLabel}</b><span>{epochModelIds.length > 0 ? `В текущий SHADOW-портфель допущено моделей: ${number(epochModelIds.length)}` : "Atlas пока не сформировал портфель кандидатов"}</span></div><strong className={portfolioNet > 0 ? "positive" : portfolioNet < 0 ? "negative" : "neutral"}>{money(runtime?.multi_model_ledger?.net_pnl_usdt)}</strong><small>общая прибыль текущего допущенного состава после комиссий</small></div><div className="decisionGrid"><Metric label="Моделей в текущем портфеле" value={number(epochModelIds.length)} hint="только прошедшие текущий отбор" /><Metric label="Доказательство портфеля" value={roundTripProgress} hint="полных входов и выходов из 20" /><Metric label="Предварительно прибыльных" value={number(profitableCandidates)} hint="ещё проходят отдельные проверки" /><Metric label="Подтверждённых" value={number(validatedProfitableCandidates)} hint="получили полное forward-доказательство" /></div><div className="portfolioContext"><div><span>Лидер исследовательского рейтинга</span><strong className={leaderProfitable ? "positive" : (leaderReturn ?? 0) < 0 ? "negative" : "neutral"}>{pct(leaderReturn)}</strong></div><div><span>{leaderName}</span><strong>{number(leaderCompleted)} сделок · просадка {dd(leaderDrawdown)}</strong></div><p>Рейтинг отдельной модели и допуск в общий портфель — разные проверки. В главный денежный итог выше не включены убыточные исследовательские модели.</p></div><p className="truthNote"><b>Как читать:</b> сначала смотрите, сколько моделей допущено и подтверждено, затем — общий результат после комиссий и прогресс 20 полных сделок. Процент отдельной модели нужен для сравнения кандидатов.</p></section>
    ...
    {tab === "trading" && <div className="page"><div className="pageTitle"><span className="eyebrow">ТОРГОВЛЯ</span><h1>Сделки ведущей модели</h1><p>Здесь показана виртуальная работа модели {leaderName}. Реальные деньги не используются.</p></div><div className="modeNotice"><b>Виртуальный режим</b><span>Реальные ордера не отправляются</span></div><section className="orientationNotice"><b>На что смотреть</b><p><strong>{pct(leaderReturn)}</strong> — итог закрытых сделок ведущей модели. Изменения открытых позиций ниже — временные и не являются итоговой прибылью.</p></section><section className="tradeSummary"><Metric label="Результат ведущей модели" value={pct(leaderReturn)} tone={leaderCompleted === 0 ? "neutral" : (leaderReturn ?? 0) >= 0 ? "positive" : "negative"} /><Metric label="Завершённые сделки модели" value={number(leaderCompleted)} /><Metric label="Открытые позиции модели" value={number(positions.length)} /></section><section className="section"><h2>Открытые виртуальные позиции</h2><p className="note">Результат позиции меняется до её закрытия. Он показан для наблюдения, а не как доказанная прибыль.</p>{positions.length === 0 ? <p className="empty">Сейчас открытых позиций нет.</p> : <div className="positionsList">{positions.map(([symbol, item]) => <PositionCard key={symbol} symbol={symbol} item={item} />)}</div>}</section><section className="section"><h2>Наблюдаемые рынки</h2><p className="note">Широкий список ускоряет поиск возможностей, но не означает автоматический допуск каждой монеты к торговле.</p><div className="marketList">{symbols.length === 0 ? <p className="empty">Данные о рынках пока не получены.</p> : symbols.map(([symbol, item]) => <article key={symbol}><div><b>{coin(symbol)}</b><small>{number(item.completed_trades)} завершённых сделок</small></div><strong className={(item.completed_trades ?? 0) === 0 ? "neutral" : (item.return ?? 0) >= 0 ? "positive" : "negative"}>{pct(item.return)}</strong><Badge status={item.market_audit?.status} /></article>)}</div></section></div>}
