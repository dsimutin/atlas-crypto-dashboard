...
type Runtime = {
  ...
  research_shadow_paper_eligible?: number;
  research_candidate_funnel?: {
    generated?: number; positive_rank_ic?: number; positive_symbol_breadth?: number;
    positive_after_costs?: number; enough_trades?: number; stable_parameter_plateau?: number;
  ...
  nautilus_replay_generated_at?: string;
  nautilus_replay_source_events?: number;
  nautilus_replay_instruments?: Array<{ instrument_id: string; source_bars: number; difference_usdt: string; passed: boolean }>;
  nautilus_replay_engine_summary?: { orders?: number; positions_closed?: number; account_difference_usdt?: string; passed?: boolean };
...
function Home({ runtime, fresh, now }: { runtime: Runtime | null; fresh: boolean; now: number }) {
  ...
  return <div className="screenBody">
    ...
    <section className="leaderCard"><span>{leader?.eligible_for_live_rank ? "ЛУЧШАЯ ПРОВЕРЕННАЯ МОДЕЛЬ СЕЙЧАС" : "ТЕКУЩИЙ КАНДИДАТ · РЕЗУЛЬТАТ ЕЩЁ НЕ ДОКАЗАН"}</span><div className="leaderTitle"><strong>{leader?.model_id ?? "Кандидат ещё не определён"}</strong><b>{leader?.eligible_for_live_rank ? `${(leader.median_return * 100).toFixed(2)}% OOS` : "результата ещё нет"}</b></div><code>{leader?.expression ?? "Нужен первый допустимый переход"}</code><div className="leaderMetrics"><small>Просадка <b>{leader?.eligible_for_live_rank ? `${(leader.maximum_drawdown * 100).toFixed(2)}%` : "ещё не измерена"}</b></small><small>Переходы позиции <b>{n(leader?.transitions)} / {required}</b></small><small>Свечи текущей модели <b>{n(leader?.minimum_live_bars)} / {n(governor?.required_live_bars)}</b></small></div><p>Сервер собирает данные с {runtime?.first_observation_at?.replace("T", " ").slice(0, 19) ?? "—"}; доказательство текущей модели началось {governor?.model_evidence_started_at?.replace("T", " ").slice(0, 19) ?? "ещё не началось"}. Осталось минимум {n(remainingBars)} свечей и {n(remainingTransitions)} переходов.</p></section>
...
function Results({ runtime }: { runtime: Runtime | null }) {
  ...
  return <div className="screenBody standalone"><h2 className="pageTitle">Что уже произошло</h2>
    ...
    <section className="statsCard"><h3>Нативный execution replay NautilusTrader</h3><div className="statsGrid"><div><span>Итог</span><strong className={runtime?.nautilus_replay_status === "PASSED" ? "positive" : "negative"}>{runtime?.nautilus_replay_status ?? "Нет проверки"}</strong></div><div><span>События</span><strong>{n(runtime?.nautilus_replay_source_events)}</strong></div><div><span>Рынки</span><strong>{n(runtime?.nautilus_replay_instruments?.length)}</strong></div><div><span>Ордера replay</span><strong>{n(runtime?.nautilus_replay_engine_summary?.orders)}</strong></div><div><span>Расхождение</span><strong>{runtime?.nautilus_replay_engine_summary?.account_difference_usdt ?? "—"} USDT</strong></div></div>{runtime?.nautilus_replay_instruments?.map(item=><div className="evidenceRow" key={item.instrument_id}><b>{item.instrument_id}</b><span>{n(item.source_bars)} свечей · разница {item.difference_usdt} USDT</span><i className={item.passed ? "pass" : "fail"}>{item.passed ? "Совпало" : "Расхождение"}</i></div>)}<small>Rust-native движок независимо исполняет экспортированные цели на validation-свечах и сверяет PnL с расчётом Atlas. API-ключи не используются, доступ к биржевым ордерам закрыт; bar-only replay пока не моделирует стакан и внутрисвечное проскальзывание.</small></section>
    ...
    <section className="statsCard"><h3>Воронка проверки новых гипотез</h3><div className="statsGrid"><div><span>Сгенерировано</span><strong>{n(runtime?.research_candidate_funnel?.generated ?? runtime?.research_generated_hypotheses)}</strong></div><div><span>Положительный rank IC</span><strong>{n(runtime?.research_candidate_funnel?.positive_rank_ic)}</strong></div><div><span>Работает ≥ 4 символов</span><strong>{n(runtime?.research_candidate_funnel?.positive_symbol_breadth)}</strong></div><div><span>Плюс после расходов</span><strong>{n(runtime?.research_candidate_funnel?.positive_after_costs)}</strong></div><div><span>Достаточно сделок</span><strong>{n(runtime?.research_candidate_funnel?.enough_trades)}</strong></div><div><span>Устойчивое плато</span><strong>{n(runtime?.research_candidate_funnel?.stable_parameter_plateau)}</strong></div><div><span>Допущено в SHADOW PAPER</span><strong>{n(runtime?.research_shadow_paper_eligible)}</strong></div><div><span>Прошло DSR/PBO</span><strong>{n(runtime?.research_candidate_funnel?.selection_bias_audit)}</strong></div><div><span>Допущено дальше PAPER</span><strong>{n(runtime?.research_candidate_funnel?.accepted ?? runtime?.research_accepted_hypotheses)}</strong></div></div><small>Устойчивые кандидаты могут собирать только независимые forward OOS-свечи и виртуальные переходы. DSR/PBO остаётся обязательным для любого продвижения дальше PAPER; доступ к ордерам закрыт.</small></section>
