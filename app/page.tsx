...
type Runtime = {
  ...
  research_lab_top_candidates?: Array<{
    parameters: { family: string; horizon?: number; fast: number; slow: number; threshold: number };
...
function Results({ runtime }: { runtime: Runtime | null }) {
  return <div className="screenBody standalone"><h2 className="pageTitle">Что уже произошло</h2>
    ...
    <section className="statsCard"><h3>Ускоритель стратегий</h3><div className="statsGrid"><div><span>Состояние</span><strong>{runtime?.research_lab_status === "READY" ? "Завершён" : "Ещё не запускался"}</strong></div><div><span>Запущено вариантов</span><strong>{n(runtime?.research_lab_tested_configs)}</strong></div><div><span>Досрочно отсечено</span><strong>{n(runtime?.research_lab_early_stopped_configs)}</strong></div><div><span>Полностью проверено</span><strong>{n(runtime?.research_lab_completed_configs)}</strong></div><div><span>Монет</span><strong>{n(runtime?.research_lab_symbols?.length)}</strong></div><div><span>Расходы в тесте</span><strong>{n(runtime?.research_lab_cost_bps)} bps</strong></div></div>{runtime?.research_lab_top_candidates?.slice(0, 3).map((item, index)=><div className="evidenceRow" key={`${item.parameters.family}-${index}`}><b>{item.parameters.family} · {(item.parameters.horizon ?? 1) * 5} мин</b><span>validation {(item.validation.median_return * 100).toFixed(2)}% · сделок {item.validation.trades} · прибыльных монет {item.validation.profitable_symbols}</span><i className={item.validation_score > 0 ? "pass" : "fail"}>{item.validation_score > 0 ? "Кандидат" : "Отклонить"}</i></div>)}<small>Long и short проверяются симметрично на 5/15/60 минутах. Лаборатория не видит ключи и ордера. Holdout не вскрыт: {runtime?.research_lab_holdout_read === false ? "да" : "нет данных"}. Доступ к торговле: {runtime?.research_lab_execution_allowed ? "ошибка" : "закрыт"}.</small></section>
