...
export default function Page() {
  ...
  return <main><div className="appShell">
    ...
    {tab === "home" && <div className="page homePage">
      ...
      <section className="section"><div className="sectionHead"><div><span className="eyebrow">ПАРАЛЛЕЛЬНЫЙ ПОРТФЕЛЬ</span><h2>Совокупный результат всех моделей</h2></div><Badge status="COLLECTING_DATA" label="Только SHADOW" /></div><p className="note">Порог наблюдений — не цель заработка, а минимум для честного вывода. Если после него PnL отрицательный, Atlas автоматически ищет замену моделям, а не продолжает ждать.</p><div className="riskGrid"><Metric label="Активные модели" value={number(runtime?.multi_model_portfolio?.active_models)} /><Metric label="Допущено прибыльных" value={number(runtime?.multi_model_portfolio?.eligible_profitable_models?.length)} /><Metric label="Открытые цели" value={number(runtime?.multi_model_portfolio?.allocation_count)} /><Metric label="Результат после расходов" value={money(runtime?.multi_model_ledger?.net_pnl_usdt)} tone={portfolioNet > 0 ? "positive" : "negative"} /><Metric label="Наблюдения" value={`${number(portfolioGate?.observations)} — порог ${number(portfolioGate?.required_observations)} пройден`} /><Metric label="Изменения позиций" value={`${number(portfolioGate?.target_transitions)} — минимум ${number(portfolioGate?.required_target_transitions)} пройден`} /><Metric label="Следующее действие" value={runtime?.stall_acceleration?.status === "ACTIVE" ? "Идёт поиск замены" : "Контроль результата"} /></div><Technical><p>Status: {runtime?.multi_model_portfolio?.status ?? "—"}; forward gate: {portfolioGate?.status ?? "—"}</p><p>Причина ускорения: {runtime?.stall_acceleration?.activation_reason ?? "нет"}; {runtime?.stall_acceleration?.reason ?? "—"}</p><p>Signals: {number(runtime?.multi_model_portfolio?.signal_count)}; risk: {runtime?.multi_model_portfolio?.total_risk_usdt ?? "0"} / {runtime?.multi_model_portfolio?.portfolio_risk_limit_usdt ?? "—"} USDT</p><p>Gross: {money(runtime?.multi_model_ledger?.gross_pnl_usdt)}; fees: {money(runtime?.multi_model_ledger?.fees_usdt)}</p><p>Requested trials: {number(runtime?.stall_acceleration?.requested_trials)}; new market slots: {number(runtime?.stall_acceleration?.requested_new_market_slots)}; retry: {number(runtime?.stall_acceleration?.retry_hours)} h</p><p>Execution allowed: {String(runtime?.multi_model_portfolio?.execution_allowed ?? false)}; Demo allowed: {String(runtime?.multi_model_portfolio?.demo_allowed ?? false)}</p></Technical></section>
